...
(function() {
    ...
    /**
     * Read hop parameters from the URL and apply them before the initial data load.
     * Mirrors the existing campaign_id param pattern.
     * 
     * Supported URL params:
     *   active_tab     - Bootstrap tab ID to switch to
     *   campaign_id    - Campaign filter value
     *   company_name   - Pre-fill account intelligence search
     *   nps_type       - NPS type filter (promoter / passive / detractor)
     *   balance_filter - Account balance filter
     */
    function applyHopParams() {
        ...
        if (activeTab) {
            ...
            if (tabEl) {
                ...
                console.log('🔗 Hop: activated tab', activeTab);
            }
        }
        ...
        if (campaignId) {
            ...
            if (campaignSelect) {
                ...
                console.log('🔗 Hop: pre-selecting campaign_id', campaignId);
            }
        }
        ...
        if (companyName) {
            ...
            if (accountSearch) {
                accountSearch.value = companyName;
                accountSearch.dispatchEvent(new Event('keyup'));
                console.log('🔗 Hop: pre-filling company_name', companyName);
            }
        }

        const balanceFilter = params.get('balance_filter');
        if (balanceFilter) {
            const balSelect = document.getElementById('accountBalanceFilter');
            if (balSelect) {
                balSelect.value = balanceFilter;
                balSelect.dispatchEvent(new Event('change'));
                console.log('🔗 Hop: pre-selecting balance_filter', balanceFilter);
            }
        }

        const npsType = params.get('nps_type');
        if (npsType) {
            const npsSelect = document.getElementById('npsTypeFilter');
            if (npsSelect) {
                npsSelect.value = npsType;
                npsSelect.dispatchEvent(new Event('change'));
                console.log('🔗 Hop: pre-selecting nps_type', npsType);
            } else {
                const npsBtn = document.querySelector('[data-nps-filter="' + npsType + '"]');
                if (npsBtn) {
                    npsBtn.click();
                    console.log('🔗 Hop: clicked nps filter button for', npsType);
                }
            }
        }
    }
