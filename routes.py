...
@app.route('/dashboard/whiteboard')
@require_business_auth
def whiteboard():
    """Whiteboard — shared landing destination for all hops (Business users only)"""
    current_business_user = get_current_business_user()
    if not current_business_user:
        from feature_flags import feature_flags
        if feature_flags.auth_soft_fallback_enabled:
    ...
    return render_template(
        ...
        is_business_authenticated=True,
        business_user_name=business_user_name,
        business_user_email=current_business_user.email,
        business_user_id=current_business_user.id,
        business_account_id=business_account_id,
        branding_context=branding_context,
    )
...
@app.route('/api/whiteboard/state', methods=['GET'])
@require_business_auth
def whiteboard_state_get():
    """Retrieve the saved whiteboard query state for the current user."""
    try:
        ...
        record = WhiteboardState.query.filter_by(
            business_user_id=current_business_user.id
        ).first()

        queries = record.queries if record else []
        return jsonify({'queries': queries})

    except Exception as e:
        logger.error(f"Error retrieving whiteboard state: {e}")
        return jsonify({'error': 'Failed to retrieve whiteboard state'}), 500


@app.route('/api/whiteboard/state', methods=['POST'])
@require_business_auth
def whiteboard_state_save():
    """Save the whiteboard query state for the current user."""
    try:
        from models import WhiteboardState
        current_business_user = get_current_business_user()
        if not current_business_user:
        ...
        MAX_CARDS = 100
        ALLOWED_CARD_TYPES = {'account', 'response'}
        ALLOWED_DESCRIPTOR_KEYS = {
            'card_type', 'company_name', 'campaign_id', 'campaign_name',
            'source_label', 'company_url', 'respondent', 'nps_type', 'response_url'
        }

        if len(queries) > MAX_CARDS:
            return jsonify({'error': f'Too many cards (max {MAX_CARDS})'}), 400

        sanitized = []
        for q in queries:
            if not isinstance(q, dict):
                continue
            if q.get('card_type') not in ALLOWED_CARD_TYPES:
                continue
            clean = {k: str(v)[:512] for k, v in q.items() if k in ALLOWED_DESCRIPTOR_KEYS and v is not None}
            if 'card_type' not in clean:
                continue
        ...
        return jsonify({'error': 'Failed to save whiteboard state'}), 500
