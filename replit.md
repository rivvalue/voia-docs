...
-   **Conversational Surveys (VOÏA)**: AI-powered (GPT-4o) natural language interface with advanced personalization and a hybrid prompt architecture for dynamic question generation and structured data extraction. Features role-based persona templates, intelligent role mapping, anonymization guards, and multilingual support. Enhanced AI prompt personalization with a structured context block for full business metadata. Persistent conversation state storage ensures data recovery. **Bug fix (Nov 21, 2025)**: Fixed welcome message language detection in `generate_welcome_message()` - French campaigns now display "Bonjour..." greeting instead of English "Hi..." (bug existed since Sept 2, 2025). **Production hotfixes (Nov 2025)**: Fixed 3 critical bugs in ai_conversational_survey.py (f-string errors, PII-masked logging for 73 print statements, improved NPS extraction regex); fixed CORS/CSRF demo mode bug; fixed dashboard campaign filter listener; fixed tenure pre-population from participant data.
...
-   **Security**: Token-based authentication, duplicate response prevention, enhanced rate limiting, robust input validation, CSRF/XSS protection, and Sentry integration. **CORS/CSRF Fix (Nov 2025)**: Resolved demo mode conversational survey bug where CORS configuration blocked X-CSRFToken header, causing "Security token validation failed" errors - added X-CSRFToken to allowed CORS headers.
...
-   **License Management System**: Enterprise-ready license management with usage tracking and enforcement. **Security fix (Nov 2025)**: Changed license enforcement from "fail open" to "fail closed" - system now denies campaign activation when license checks fail, protecting revenue and preventing unlimited usage during system errors. **Bug fix (Nov 21, 2025)**: Fixed `apply_license_template()` missing `max_invitations_per_campaign` assignment - custom Pro licenses now correctly persist all 4 limit fields instead of reverting invitation limit to template default.
