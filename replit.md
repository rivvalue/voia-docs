...
-   **Classic Survey System (Phase 1 - Foundation)**: Dual survey type architecture supporting both AI-powered conversational (VOÏA) and structured classic surveys. Campaign-level survey_type field (default: 'conversational') with SurveyTemplate and ClassicSurveyConfig models for template-based survey customization. Survey type locked after campaign activation via frozen_at timestamp. Bilingual (EN/FR) support throughout. New SurveyResponse fields: csat_score, ces_score, loyalty_drivers, recommendation_status.
